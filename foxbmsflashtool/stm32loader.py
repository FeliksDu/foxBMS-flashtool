...
class CommandInterface:
    ...
    def open(self, aport='/dev/tty.usbserial-ftCYPMYJ', abaudrate=230400) :
    ...
    def cmdGet(self):
        if self.cmdGeneric(0x00):
            ...
            logging.debug("    Available commands: "+", ".join(dat))
    ...
    def cmdEraseMemory(self, sectors = None):
        ...
        if self.cmdGeneric(0x43):
            ...
            logging.debug("    Erase memory done")
        else:
    ...
    def cmdExtendedEraseMemory(self):
        if self.cmdGeneric(0x44):
            ...
            self.sp.timeout = tmp
            logging.debug("    Extended Erase memory done")
        else:
            raise CmdException("Extended Erase memory (0x44) failed")

    def cmdWriteProtect(self, sectors):
        if self.cmdGeneric(0x63):
            logging.debug("*** Write protect command")
            self.sp.write(chr((len(sectors)-1) & 0xFF))
            crc = 0xFF
            for c in sectors:
                crc = crc ^ c
...
# Complex commands section
    ...
    def readMemory(self, addr, lng):
        ...
        while lng > 256:
            if usepbar:
                ...
                logging.info("Read %(len)d bytes at 0x%(addr)X" % {'addr': addr, 'len': 256})
    ...
    def writeMemory(self, addr, data):
        ...
        while lng > 256:
            ...
            logging.info("[%d/%d] - hi" % (alllng - lng, alllng))
...
# http://www.python.org/doc/2.5.2/lib/module-getopt.html
    ...
    logging.debug("Open port %(port)s, baud %(baud)d" % {'port':conf['port'], 'baud':conf['baud']})
    try:
        try:
            cmd.initChip()
        except:
        ...
        logging.info("Bootloader version %X" % bootversion)
        id = cmd.cmdGetID()
        logging.info("Chip id: 0x%x (%s)" % (id, chip_ids.get(id, "Unknown")))
#    cmd.cmdGetVersion()
#    cmd.cmdGetID()
